<%@ Page Title="" Language="C#" MasterPageFile="~/MyMater.Master" AutoEventWireup="true" CodeBehind="webCashExpensePosted.aspx.cs" Inherits="ADAM.Account.webCashExpensePosted" %>

<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="pageAddress" runat="server">
    <h1>
        <asp:Label ID="lblPageName" CssClass="lblPageName" runat="server" Text="شاشة ترحيل سندات الصرف" Font-Size="X-Large" Font-Underline="true"></asp:Label></h1>
    <table class="menu">
        <tr>
            <td>
...
<asp:Content ID="Content3" ContentPlaceHolderID="PageData" runat="server">
    ...
    <asp:GridView ID="gvJorunal" Width="100%" CssClass="gv" runat="server" AutoGenerateColumns="False" DataSourceID="dbgvJournal" DataKeyNames="JournalHeaderId" OnSelectedIndexChanged="gvJorunal_SelectedIndexChanged">
        <Columns>
            <asp:BoundField DataField="DocNo" HeaderText="رقم سند الصرف" SortExpression="DocNo" />
            <asp:BoundField DataField="DocDate" DataFormatString="{0:dd-MM-yyyy}" HeaderText="تاريخ سند الصرف" SortExpression="DocDate" />
    ...
    <table>
        <tr>
            ...
            <td>
                <asp:SqlDataSource ID="dbgvJournal" runat="server" ConnectionString="<%$ ConnectionStrings:ADAMConnectionString %>" SelectCommand="SELECT JournalHeader.Id AS JournalHeaderId, JournalHeader.JournalCode, JournalHeader.JournalDate, JournalHeader.JournalType, JournalHeader.Note, JournalHeader.Posted, JournalHeader.DocId, JournalType.Id AS JournalTypeId, JournalType.JournalTypeName, DocumentHeader.DocNo, DocumentHeader.DocDate, DocumentHeader.DocType FROM JournalHeader INNER JOIN JournalType ON JournalHeader.JournalType = JournalType.Id INNER JOIN DocumentHeader ON JournalHeader.DocId = DocumentHeader.Id WHERE (JournalType.Id = 6) AND (JournalHeader.Posted = 0) AND (DocumentHeader.DocType = 1)">
                </asp:SqlDataSource>
            </td>
            <td>
