<%@ Page Title="" Language="C#" MasterPageFile="~/MyMater.Master" AutoEventWireup="true" CodeBehind="webAskedOrderReport.aspx.cs" Inherits="ADAM.SalesReport.webAskedOrderReport" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="pageAddress" runat="server">
    <h1>
        <asp:Label ID="lblPageName" runat="server" Text="تقرير أذن صرف عهده مسعر" CssClass="lblPageName" Font-Size="X-Large" Font-Underline="true"></asp:Label></h1>
</asp:Content>
<asp:Content ID="Content3" ContentPlaceHolderID="PageData" runat="server">
    ...
    <asp:SqlDataSource ID="dbExchangePrice" runat="server" ConnectionString="<%$ ConnectionStrings:ADAMConnectionString %>" SelectCommand="SELECT ExchangeRequestHeaderData.ExchangeRequestNo, ExchangeRequestHeaderData.ExchangeRequestDate, Items.Name AS ItemName, ExchangeRequestDetailsData.Qty, ExchangeRequestDetailsData.FreeQty, ExchangeRequestDetailsData.Bounce, ItemUnit.Name AS ItemUnitName, ItemStatus.ItemStatus, ItemType.Name AS ItemTypeName, ProductionLine.productionLineName, SexData.Sex, division.Name AS divisionName, EmployeeData.Code AS EmpCode, EmployeeData.FirstName + ' ' + EmployeeData.LastName AS EmpName, ExchangeRequestHeaderData.OrderType, ExchangeRequestDetailsData.ExchangeRequestHeaderDataId, Items.Code AS ItemCode, ClientData.Code AS ClientCode, ClientData.FirstName + '  ' + ClientData.LastName AS ClientName, ClientData.FirstPhone, ClientData.FirstMobile, ClientData.Address FROM ExchangeRequestDetailsData INNER JOIN ExchangeRequestHeaderData ON ExchangeRequestDetailsData.ExchangeRequestHeaderDataId = ExchangeRequestHeaderData.Id INNER JOIN ExchangeRequestPricing ON ExchangeRequestDetailsData.Id = ExchangeRequestPricing.ExchangeRequestDetailsId INNER JOIN Items ON ExchangeRequestDetailsData.ItemId = Items.Id INNER JOIN ItemColorSelected ON Items.Id = ItemColorSelected.ItemId INNER JOIN ItemUnit ON Items.ItemunitId = ItemUnit.Id INNER JOIN ItemType ON ExchangeRequestHeaderData.ItemTypeId = ItemType.Id AND Items.ItemTypeId = ItemType.Id INNER JOIN ItemStatus ON Items.ItemStatus = ItemStatus.Id INNER JOIN SexData ON Items.Sex = SexData.Id INNER JOIN ProductionLine ON Items.ProductionLineId = ProductionLine.Id INNER JOIN division ON ExchangeRequestHeaderData.DivisionId = division.Id INNER JOIN EmployeeData ON ExchangeRequestHeaderData.EmpId = EmployeeData.Id AND division.Id = EmployeeData.DivisionId INNER JOIN ClientData ON ExchangeRequestHeaderData.ClientId = ClientData.Id WHERE (ExchangeRequestHeaderData.OrderType = 12)"></asp:SqlDataSource>
