<%@ Page Title="" Language="C#" MasterPageFile="~/MyMater.Master" AutoEventWireup="true" CodeBehind="webExchangeRequestReport.aspx.cs" Inherits="ADAM.StoreReport.webExchangeRequestReport" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="pageAddress" runat="server">
    <h1>
        <asp:Label ID="lblPageName" runat="server" Text="تقرير طلب الصرف" CssClass="lblPageName" Font-Size="X-Large" Font-Underline="true"></asp:Label></h1>
</asp:Content>
<asp:Content ID="Content3" ContentPlaceHolderID="PageData" runat="server">
    <div style="position: relative; padding-right: 50px">
        ...
        <table>
            <tr>
                <td>
                    <asp:SqlDataSource ID="dbExchangeRequest" runat="server" ConnectionString="<%$ ConnectionStrings:ADAMConnectionString %>" SelectCommand="SELECT 0 AS Id, '----' AS MovementName UNION SELECT Id, MovementName FROM MovmentName WHERE (Id &gt; 7) AND (Id &lt; 13) or(Id=17)or(Id=22)"></asp:SqlDataSource>
                </td>
                <td>
                ...
                <td>
                    <asp:SqlDataSource ID="dbExcahnge" runat="server" ConnectionString="<%$ ConnectionStrings:ADAMConnectionString %>" SelectCommand="SELECT ExchangeRequestHeaderData.ExchangeRequestNo, ExchangeRequestHeaderData.ExchangeRequestDate, Items.Code, Items.Name, ItemType.Name AS ItemTypeName, division.Name AS divisionName, EmployeeData.FirstName + ' ' + EmployeeData.LastName AS EmpName, ExchangeRequestDetailsData.Qty, SexData.Sex, ItemUnit.Name AS ItemUnitName, Items.LimitQty, ItemStatus.ItemStatus, Items.Specification, ProductionLine.productionLineName, Items.ItemTypeId, ItemColor.ColorName, ExchangeRequestHeaderData.OrderType, ExchangeRequestDetailsData.FreeQty, ExchangeRequestDetailsData.Bounce, ExchangeRequestDetailsData.Id AS ExchangeRequestDetailsDataId, ExchangeRequestDetailsData.Status FROM ExchangeRequestDetailsData INNER JOIN ExchangeRequestHeaderData ON ExchangeRequestDetailsData.ExchangeRequestHeaderDataId = ExchangeRequestHeaderData.Id INNER JOIN Items ON ExchangeRequestDetailsData.ItemId = Items.Id INNER JOIN ItemType ON Items.ItemTypeId = ItemType.Id AND ExchangeRequestDetailsData.ItemTypeId = ItemType.Id INNER JOIN division ON ExchangeRequestHeaderData.DivisionId = division.Id INNER JOIN EmployeeData ON ExchangeRequestHeaderData.EmpId = EmployeeData.Id AND division.Id = EmployeeData.DivisionId INNER JOIN SexData ON Items.Sex = SexData.Id INNER JOIN ItemUnit ON Items.ItemunitId = ItemUnit.Id INNER JOIN ItemStatus ON Items.ItemStatus = ItemStatus.Id INNER JOIN ProductionLine ON Items.ProductionLineId = ProductionLine.Id INNER JOIN ItemColor ON ExchangeRequestDetailsData.ItemColorId = ItemColor.Id WHERE (ExchangeRequestHeaderData.OrderType = @OrderType)">
                        <SelectParameters>
